...
class AiterMxfp4BackendAdapter(FormatBackendAdapter):
    ...
    def __init__(self, *, use_fp6_for_overrides: bool = False, **kwargs):
        ...
        if use_fp6_for_overrides:
            ...
            self.auxiliary_state_semantics = "replicated_scales_and_mxfp6_provenance"
...
class AiterMxfp6BackendAdapter(FormatBackendAdapter):
    ...
    parameter_semantics = "packed_weight_parameter"
    auxiliary_state_semantics = "persistent_scale_and_provenance_buffers"
    serialization = "packed_state_supported_not_portable"
...
def select_format_backend(
    ...
    *,
    ...
    if format_value in {"fp6", "fp4_fp6"}:
        ...
        if hybrid:
            raise error(
                "AITER MXFP6 cannot be combined with the hybrid FP8/FP4 "
                "GEMM schedule"
            )
        if format_value == "fp6":
